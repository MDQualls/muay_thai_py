...
A local web app that generates stylized Muay Thai fighter profile cards and posts them to Instagram. Enter a fighter's name, and the app scrapes their record, enriches it with Claude AI, renders a 1080×1080 JPEG card, and publishes it to Instagram — all from a browser at `localhost:8000`.
...
| `server/renderer.py` | Renders Jinja2 card template to JPEG via Playwright |
| `server/uploader.py` | Uploads card JPEG to Cloudflare R2, returns public URL |
